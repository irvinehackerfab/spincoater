...
use crate::{
    APP_PERIOD,
    gpio::display::terminal::channel::{TERMINAL_CHANNEL_SIZE, TuiEvent, send_event_or_report},
};
...
/// The the current plate RPM, which is regularly updated by [`read_rpm`]
pub static PLATE_RPM: AtomicU16 = AtomicU16::new(0);

/// Every so often, this task sends the number of plate revolutions per minute to the terminal.
/// It only sends the RPM if at least half a motor revolution has occurred.
///
/// # Panics
/// This task panics if more than [`u32::MAX`] milliseconds has passed after 20 milliseconds has passed,
/// which should never happen.
///
/// It also panics if the rpm exceeds [`u32::MAX`].
#[embassy_executor::task]
pub async fn read_rpm(to_terminal: Sender<'static, NoopRawMutex, TuiEvent, TERMINAL_CHANNEL_SIZE>) {
    ...
    loop {
        ...
        if motor_revolutions_doubled != 0 {
            let time_ms =
                u32::try_from(time.as_millis()).expect("20 milliseconds should fit in a u32.");
            ...
            send_event_or_report(&to_terminal, TuiEvent::RpmValue).await;
        }
